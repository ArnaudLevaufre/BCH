        ...
        \section{Look up table}
            ...
            \begin{figure}[H]
                ...
                \begin{tikzpicture}[>=stealth, node distance=3cm, scale=0.7, every node/.style={scale=0.7}, circuit logic US]
                    ...
                    \draw[<-] (p2counter.Ld) -- +(-2, 0) node [anchor=east] {Raz\_P2};

                    \draw[->] (p1counter.Q) -- +(1, 0) -- +(1, -4) -- +(-3, -4) |- (p2counter.Data);
                    \draw[->] (p1counter.Q) -- +(1, 0) -- +(1, -4) |- (lut.Pone);
                    \draw[->] (p1counter.Q) -- +(1, 0) |- (p1max.west);

                    \draw[->] (p2counter.Q) -- +(1, 0) |- (lut.Ptwo);
                    \draw[->] (p2counter.Q) -- +(1, 0) |- (p2max.west);

                    \draw[->] (lut.Sone) -- +(1, 0) |- (xor.input 1);
                    \draw[->] (lut.Sone) -- +(1, 0) |- (cmp1.west);
                    \draw[->] (lut.Stwo) -- +(1, 0) |- (xor.input 2);

                    \draw[->] (xor.output) -- +(0.25, 0) |- (cmp2.west);

                    \draw[->] (syn.Q) -| (cmp1.north);
                    \draw[->] (syn.Q) -| (13.33, 7.5) -| (cmp2.north);
                    ...
                    \draw[<-] (p1max.north) -- +(0, 1) node [anchor=south] {30};
                    \draw[<-] (p2max.south) -- +(0, -1) node [anchor=north] {30};
            ...
            \begin{figure}[H]
                \begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=4cm]
                    ...
                    \path[->] (E1) edge node {start\_lut / raz, raz\_p2, ld\_syndrome} (E2);

                    \path[->] (E2) edge [loop right] node  {p2\_max / inc\_p1, raz\_p2} (E2);
